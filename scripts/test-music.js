...
async function run() {
  ...
  try {
    ...
    delete editableRules.artistVariety;
    delete editableRules.albumVariety;
    ...
    assert.equal(editableRadio.rules.artist.artistVariety, 2, 'legacy artist spacing migrates to Balanced Artist Variety');
    assert.equal(editableRadio.rules.artist.albumVariety, 2, 'legacy album spacing migrates to Balanced Album Variety');
    ...
    assert.deepEqual(editableSchema.schema.artist.artistFocusPercent.stops, [0, 25, 50, 75, 100]);
    assert.deepEqual(editableSchema.schema.artist.songPopularityPercent.stops, [0, 25, 50, 75, 100]);
    assert.deepEqual(editableSchema.schema.artist.genreWeight.stops, [0, 1, 5, 10, 20]);
    assert.deepEqual(editableSchema.schema.artist.artistVariety.stops, [0, 1, 2, 3, 4]);
    assert.deepEqual(editableSchema.schema.artist.albumVariety.stops, [0, 1, 2, 3, 4]);
    assert.deepEqual(editableSchema.schema.artist.releaseYearRange.stops, [0, 5, 10, 20, 10000]);
    assert.deepEqual(editableSchema.schema.artist.repeatCooldownMinutes.stops, [120, 240, 480, 960, 1440]);
    assert.deepEqual(editableSchema.schema.artist.selectionRandomness.stops, [0, 0.5, 1, 3, 10]);
    assert.deepEqual(editableSchema.schema.artist.ratingInfluence.stops, [0, 1, 2, 3, 4]);
    assert.equal(Object.hasOwn(editableSchema.schema.artist, 'unrelatedTrackMultiplier'), false, 'Outside Variety is removed');
    for (const modeSchema of Object.values(editableSchema.schema)) {
      for (const [key, spec] of Object.entries(modeSchema)) assert.equal(spec.stops?.length, 5, `${key} should have exactly five stops`);
    }
    ...
    assert.equal(editableRadio.setRule('artist', 'artistFocusPercent', 72).artist.artistFocusPercent, 75, 'Artist Focus snaps to one of five stops');
    ...
    assert.equal(editableRadio.setRule('artist', 'songPopularityPercent', 72).artist.songPopularityPercent, 75, 'Song Popularity snaps to one of five stops');
    assert.equal(editableRadio.setRule('artist', 'artistVariety', 4).artist.artistVariety, 4);
    assert.equal(editableRadio.setRule('artist', 'albumVariety', 4).artist.albumVariety, 4);
    assert.equal(editableRadio.setRule('artist', 'releaseYearRange', 13).artist.releaseYearRange, 10);
    assert.equal(editableRadio.setRule('radio', 'genreWeight', 100).radio.genreWeight, 20);
    ...
    assert.equal(laneRadio.choose([laneRelated, laneOutside], laneSeed, 'radio').id, laneRelated.id, 'radio chooses only from the acceptable related pool');
    assert.equal(laneRadio.choose([laneOutside], laneSeed, 'radio'), null, 'radio waits instead of falling back to unrelated music');
    assert.equal(laneRadio.getLastDecision().counts.excludedForRelevance, 1);
    assert.equal(editableRadio.resetRules('radio').radio.genreWeight, 5);
    assert.equal(editableRadio.rules.artist.version, 7);
    ...
    assert.equal(decision.settings.artistVariety, 2);
    assert.equal(decision.settings.ratingInfluence, 0, 'diagnostics record the active Ratings Matter level');
