...
- Branch: `main`
- Current published version: **0.7.16**
- Current release: 0.7.16 — personal Local Radio signals and Do Not Play enforcement.
...
`DO_NOT_PLAY=1` is an absolute exclusion for every Local Music path: Local Radio, album playback, and manual Local Radio starts. `FAVORITE=1` and MP3 ratings are copied into the local index without modifying files and are the strongest personal taste signals. Last.fm is optional and fills familiarity gaps only after a song is credible. Under Favor the Hits, WaveDeck first narrows to Favorites, 4–5 star songs, and familiar Last.fm tracks, expanding only if that pool is unavailable. Missing popularity remains neutral. When no credible candidate is available, Local Radio waits. Exact songs observe a fixed two-hour repeat wait.
...
WaveDeck 0.7.16 reads `DO_NOT_PLAY` as an absolute exclusion and copies both it and `FAVORITE` into the portable music index. The index performs one full metadata refresh after upgrade so existing unchanged files receive these fields. Favor the Hits now uses a known-good pool first: favorites, 4–5 star songs, and Last.fm-popular credible candidates; ratings and favorites outweigh public popularity.
